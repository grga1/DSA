...
public class Main {
    public static void main(String[] args) {
        ...
        while (tmp != null) {
            int sumLevo = 0, sumDesno = 0;
            int countLevo = 0, countDesno = 0;

            DLLNode<Integer> tmpLevo = tmp.pred;
            while (tmpLevo != null) {
                sumLevo += tmpLevo.element;
                countLevo++;
                tmpLevo = tmpLevo.pred;
            }
            
            DLLNode<Integer> tmpDesno = tmp.succ;
            while (tmpDesno != null) {
                sumDesno += tmpDesno.element;
                countDesno++;
                tmpDesno = tmpDesno.succ;
            }

            
            if (countLevo > 0 && countDesno > 0) {
                double avgLevo = (double) sumLevo / countLevo;
                double avgDesno = (double) sumDesno / countDesno;
