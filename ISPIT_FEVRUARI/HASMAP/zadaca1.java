...
Секој проект да се печати во формат "<time, rate>" каде на местото на time и rate треба да стои работното време и 
...
Each employee should be printed in the format "<name, age>" where in place of name and age should be the name 
and age of the employee respectively (implement this in the toString method)
Each project should be printed in the format "<time, rate>" where in place of time and rate should be the 
working hours and hourly wage of the project respectively (implement this in the toString method)
Пример/Еxample:
Влез/Input:
5  
Martin 25 3 100  
...
class Project {
    ...
    public Project(int rabVreme, int plataPoCas) {
        this.rabVreme = rabVreme;
        this.plataPoCas = plataPoCas;
    }

    @Override
    public String toString() {
        return "<" + rabVreme + ", " + plataPoCas + ">";
    }
...
public class Main {
    public static void main(String[] args) {
        ...
        int N = sc.nextInt(); // Број на вработени
        CBHT<Person, Project> mapa = new CBHT<>(10); // Хеш-табела со 10 кофички

        for (int i = 0; i < N; i++) {
            String ime = sc.next(); // Име
            int vozrast = sc.nextInt(); // Возраст
            int rabVreme = sc.nextInt(); // Работно време
            int plataCas = sc.nextInt(); // Плата по час

            Person p = new Person(ime, vozrast);
            Project z = new Project(rabVreme, plataCas);
         SLLNode<MapEntry<Person,Project>> tmp = mapa.search(p);
         if(tmp!=null){
             if(z.vkupnaPlata()>tmp.element.value.vkupnaPlata()){
                 mapa.insert(p,z);
             }
         }else mapa.insert(p,z);
        }

        System.out.println(mapa); // Печати ја табелата
    }
}
